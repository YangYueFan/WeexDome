...
//    backBtn.frame = CGRectMake(10, 44, 32, 32);
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [RHScanPermissions requestCameraPemissionWithResult:^(BOOL granted) {
        if (granted) {
            ...
            [weakSelf showError:@"   请到设置隐私中开启本程序相机权限   " withReset:NO];
        }
    }];
}
...
- (void)scanResultWithArray:(NSArray<NSString*>*)array
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"didQRReaderGetResult" object:self userInfo:@{@"result":scanResult}];
    if (self.scanBlock) {
        self.scanBlock(@{@"info":scanResult});
    }
